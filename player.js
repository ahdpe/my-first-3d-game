...
function createPlayer(scene) {
    ...
    var torso = BABYLON.MeshBuilder.CreateBox("torso", {height: torsoHeight, width: torsoWidth, depth: torsoDepth}, scene); torso.material = bodyMaterial; torso.parent = cameraTargetPoint; torso.position.y = 0; // Центр туловища = цель камеры
    ...
    scene.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnKeyDownTrigger, function (evt) { inputMap[evt.sourceEvent.key.toLowerCase()] = true; })); // Используем true/false
    scene.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnKeyUpTrigger, function (evt) { inputMap[evt.sourceEvent.key.toLowerCase()] = false; })); // Используем true/false
    ...
    // --- ИНИЦИАЛИЗАЦИЯ ---
    playerRoot.rotation.y = 0; // Игрок смотрит вперёд
    ...
    function lerpLimbsToZero(lerpFactor = 0.1) { /* ... код без изменений ... */
    ...
    scene.onBeforeRenderObservable.add(() => {
        // ==============================================================
        // !!! ВОТ ИСПРАВЛЕННАЯ СТРОКА (СТРОКА ~119) !!!
        // Используем scene.getEngine() для доступа к движку
        ...
        // 3. Горизонтальное движение и поворот
        ...
        if (isGrounded && playerState !== PlayerState.JUMP_ANTICIPATION && playerState !== PlayerState.LANDING) {
             ...
             let moveDirection = new BABYLON.Vector3(0, 0, 0); let movingX = false, movingZ = false;
             ...
             if (isMovingHorizontally) {
                 moveDirection.normalize(); targetRotationY = Math.atan2(moveDirection.x, moveDirection.z);
        ...
        switch (playerState) {
            case PlayerState.IDLE: case PlayerState.WALKING: case PlayerState.RUNNING:
                if (isMovingHorizontally) {
                    let currentAnimSpeed = walkAnimSpeed * (inputMap["shift"] ? sprintMultiplier : 1); walkAnimAngle += currentAnimSpeed * (deltaTime / 16.66);
                    targetArmXAngle = Math.sin(walkAnimAngle) * walkAnimAmplitude; targetLegBend = Math.sin(walkAnimAngle + Math.PI) * walkAnimAmplitude;
                } else { walkAnimAngle = 0; targetArmXAngle = 0; targetLegBend = 0; }
                leftShoulderPivot.rotation.x = BABYLON.Scalar.Lerp(leftShoulderPivot.rotation.x, targetArmXAngle, 0.2); rightShoulderPivot.rotation.x = BABYLON.Scalar.Lerp(rightShoulderPivot.rotation.x, -targetArmXAngle, 0.2);
                leftHipPivot.rotation.x = BABYLON.Scalar.Lerp(leftHipPivot.rotation.x, targetLegBend, 0.2); rightHipPivot.rotation.x = BABYLON.Scalar.Lerp(rightHipPivot.rotation.x, -targetLegBend, 0.2);
                leftShoulderPivot.rotation.z = BABYLON.Scalar.Lerp(leftShoulderPivot.rotation.z, 0, 0.2); rightShoulderPivot.rotation.z = BABYLON.Scalar.Lerp(rightShoulderPivot.rotation.z, 0, 0.2);
                break;
            case PlayerState.JUMP_ANTICIPATION:
                ...
                leftShoulderPivot.rotation.x = BABYLON.Scalar.Lerp(leftShoulderPivot.rotation.x, targetArmXAngle, 0.3); rightShoulderPivot.rotation.x = BABYLON.Scalar.Lerp(rightShoulderPivot.rotation.x, targetArmXAngle, 0.3);
                leftHipPivot.rotation.x = BABYLON.Scalar.Lerp(leftHipPivot.rotation.x, targetLegBend, 0.3); rightHipPivot.rotation.x = BABYLON.Scalar.Lerp(rightHipPivot.rotation.x, targetLegBend, 0.3);
                leftShoulderPivot.rotation.z = BABYLON.Scalar.Lerp(leftShoulderPivot.rotation.z, 0, 0.3); rightShoulderPivot.rotation.z = BABYLON.Scalar.Lerp(rightShoulderPivot.rotation.z, 0, 0.3);
            ...
            case PlayerState.AIRBORNE:
                ...
                lerpLimbsToZero(0.1);
                leftShoulderPivot.rotation.z = BABYLON.Scalar.Lerp(leftShoulderPivot.rotation.z, targetArmZAngle, 0.15); rightShoulderPivot.rotation.z = BABYLON.Scalar.Lerp(rightShoulderPivot.rotation.z, -targetArmZAngle, 0.15);
                leftHipPivot.rotation.x = BABYLON.Scalar.Lerp(leftHipPivot.rotation.x, targetLegBend, 0.15); rightHipPivot.rotation.x = BABYLON.Scalar.Lerp(rightHipPivot.rotation.x, targetLegBend, 0.15);
                break;
            case PlayerState.LANDING:
                ...
                leftHipPivot.rotation.x = BABYLON.Scalar.Lerp(leftHipPivot.rotation.x, targetLegBend, 0.25); rightHipPivot.rotation.x = BABYLON.Scalar.Lerp(rightHipPivot.rotation.x, targetLegBend, 0.25);
                if (stateTimer >= landingTime) {
                    playerState = isMovingHorizontally ? PlayerState.WALKING : PlayerState.IDLE; targetTorsoOffsetY = 0;
                } break;
        }
